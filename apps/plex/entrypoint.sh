...
# Plex seeds its library database by copying it out of the (read-only) Nix
# store, which would leave the copy unwritable. Seed it ourselves with sane
# permissions on first start.
dbDir="${PLEX_MEDIA_SERVER_APPLICATION_SUPPORT_DIR}/Plex Media Server/Plug-in Support/Databases"
mkdir -p "${dbDir}"
# Plex installs the same template into both the library and the blobs database.
for db in com.plexapp.plugins.library.db com.plexapp.plugins.library.blobs.db; do
    if [[ ! -f "${dbDir}/${db}" ]]; then
        echo "Seeding ${db}"
        cp --no-preserve=mode "${PLEX_MEDIA_SERVER_HOME}/Resources/com.plexapp.plugins.library.db" "${dbDir}/${db}"
        chmod 644 "${dbDir}/${db}"
    fi
done
